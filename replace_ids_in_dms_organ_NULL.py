...
#                     select_query = f"SELECT COUNT(*) FROM {table} WHERE {column} = %s AND entity_type = 1"
...
#                 if count > 0:
#                     if check_for_entity_type:
#                         update_query = f"UPDATE {table} SET {column} = %s WHERE {column} = %s AND entity_type = 1"
...
# update_ids_from_json("record_to_entity_organisation_NULL.json", tables_to_update)
...
# # Tables to update and whether to check for entity_type = 1
# tables_to_update = {
#     "leads_connections": ("entity_id", True),
#     "addresses": ("entity_id", True),
#     "ledgers_role_mapping": ("entity_id", True),
...
# def update_ids_from_json(json_file, tables_info, log_file="dms_ids_replace_org_NULL.txt", chunk_size=2000):
#     with open(json_file, "r") as f:
#         record_map = json.load(f)
...
# # Run the function for NULL-mapped records
# update_ids_from_json("records_entity_mapping_for_global_organisations_NULL.json", tables_to_update)
...
tables_to_update = {
    # "leads_connections": ("entity_id", True), on it 
    # "addresses": ("entity_id", True), on it -- done 
    # "ledgers_role_mapping": ("entity_id", True), -- done 
    # "global_entity_contacts": ("entity_id", True), -- done 
    "ledgers_role_mapping":("related_entity_id",True)

    # "leads_notes": ("organisation_id", False), on it 
    # "leads_tags": ("global_organisation_id", False), done 
    # "leads_tickets": ("global_organisation_id", False), on it 
    # "leads_transactions": ("organisation_name", False), 
...
# Logging helper
def log_message(message, log_file="update_log_null_replacing_13april.txt"):
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    with open(log_file, "a") as f:
        f.write(f"[{timestamp}] {message}\n")

# Main update logic
def update_ids_from_json(json_file, tables_info, chunk_size=2000,log_file="update_log_null_replacing_13april.txt"):
    try:
        with open(json_file, "r") as f:
            record_map = json.load(f)
        record_items = [(int(old_id), new_id) for old_id, new_id in record_map.items()]
